...
const FilterWrapper = ({children} : {children: ReactNode}) => {
    return (
        <div className={"relative px-5 cursor-pointer"}>
            <div className={"border-neutral-100 py-3 border-b"}>
                    {children}
            </div>
        </div>
    )
}
const FilterLayout = <T extends FilterTypes>({filter, children}: { filter: T, children: ReactNode }) => {
    ...
    if (filter.type === "switch") {
        layout = (
            <FilterWrapper>
            <div
                    className={"w-full flex items-center justify-between text-neutral-700"}>
                        ...
                        {filter.title}
                        {
                            filter.icon && (
                                <div className={"w-[18px] h-[18px] mr-2 flex items-center justify-center"}>
                                    <img src={filter.icon} width={18} height={18} alt={""}
        ...
        layout = (
            <>
                <FilterWrapper>
                    <div onClick={toggleIsVisible} className={"w-full flex items-center text-subtitle-strong text-neutral-700 justify-between"}>
                        {filter.title}
