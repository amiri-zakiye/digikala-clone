const ProductItemShimmer = ({Id}: {Id: string}) => {
    return (
        <div
            className="block cursor-pointer relative overflow-hidden grow py-3 px-4 lg:px-2 h-full">
            <svg aria-labelledby="l8nd94-aria" role="img" viewBox="0 0 320 530" className="m-auto w-full h-full"><title
                id="l8nd94-aria">Loading...</title>
                <rect role="presentation" x="0" y="0" width="100%" height="100%" clipPath={`url(#${Id}-diff)`}
                      style={{fill: `url(#${Id}-animated-diff)`}}></rect>
                <defs>
                    <clipPath id={`${Id}-diff`}>
